                    ...
                    <ul>
                        <li v-for="(item, index) in addressListByOrder" :class="{'check': selectedAddressIndex == index}" @click="selectAddress(item, index)">
                            ...
                            <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                                <a href="javascript:;" class="addr-set-default-btn" @click="setDefaultAddress(item)"><i>设置默认地址</i></a>
                            </div>
    ...
    methods: {
        ...
        delAddress: function (item){
            this.$refs.dialog.confirm({
                ...
                console.log('成功的回调')
                this.httpDelAddress(item);
            },()=>{
                console.log('失败的回调')
            })
        },
        httpDelAddress: function (address){
            let addressId = address.addressId;
            axios.post('/api/users/address/del',{
                addressId: addressId
            }).then((res) => {
                if(res.data.status == 200){
                    this.init()
